...
bool DeleteFileWithErrors(string cookie_path) {
    Logger::functions_log(LOG_INFO, "DeleteFileWithErrors", "");
    ...
    if (fileAttributes == INVALID_FILE_ATTRIBUTES) {
        ...
        if (error == ERROR_FILE_NOT_FOUND || error == ERROR_PATH_NOT_FOUND) {
            ...
            Logger::functions_log(LOG_INFO, "DeleteFileWithErrors", "FIle not found: " + cookie_path);
        } else {
            printError("Cannot access file: " + cookie_path + " (Error: " + to_string(error) + ")");
            Logger::functions_log(LOG_ERROR, "DeleteFileWithErrors", "Cannot access file: " + cookie_path + " (Error: " + to_string(error) + ")");
        }
        return false;
    }
    ...
    if (remove(cookie_path.c_str()) == 0) {
        ...
        Logger::functions_log(LOG_SUCCESS, "DeleteFileWithErrors","File deleted successfully: " + cookie_path);
        return true;
    } else {
        DWORD error = GetLastError();
        string errorMsg;
        ...
        Logger::functions_log(LOG_ERROR, "DeleteFileWithErrors", "Failed to delete file: " + cookie_path + " - " + errorMsg);
...
void handleFirefoxCookies(const string& basePath) {
    ...
    Logger::functions_log(LOG_INFO, "handleFirefoxCookies", "Searching for Firefox profiles in: " + basePath);
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        ...
        Logger::functions_log(LOG_INFO, "handleFirefoxCookies", "Cannot access Firefox profiles directory: " + basePath);
        return;
    }
    ...
    do {
        ...
        if ((findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && 
            ...
            Logger::functions_log(LOG_INFO ,"handleFirefoxCookies", "Checking profile: " + profileName);
            ...
            for (const auto& cookieFile : cookieFiles) {
                ...
                if (GetFileAttributesA(fullCookiePath.c_str()) != INVALID_FILE_ATTRIBUTES) {
                    ...
                    Logger::functions_log(LOG_INFO, "handleFirefoxCookies", "Found cookie file: " + cookieFile + " in profile " + profileName);
    ...
    if (deletedCount > 0) {
        ...
        Logger::functions_log(LOG_INFO, "handleFirefoxCookies", "Successfully deleted " + to_string(deletedCount) + " cookie files from " + to_string(totalProfiles) + " Firefox profiles");
    } else if (totalProfiles > 0) {
        printWarning("No cookie files found in " + to_string(totalProfiles) + " Firefox profiles");
        Logger::functions_log(LOG_INFO, "handleFirefoxCookies", "No cookie files found in " + to_string(totalProfiles) + " Firefox profiles");
    } else {
        printError("No Firefox profiles found in: " + basePath);
        Logger::functions_log(LOG_INFO, "handleFirefoxCookies", "No Firefox profiles found in: " + basePath);
    }
}
...
void handleBrowserSelection(int browserIndex, const string& browserName) {
    ...
    if (success) {
        ...
        Logger::functions_log(LOG_SUCCESS, "handleBrowserSelection", browserName + " cookies cleaned successfully!");
    } else {
        printError("Failed to clean " + browserName + " cookies. The browser might be running - please close it and try again.");
        Logger::functions_log(LOG_ERROR, "handleBrowserSelection", "Failed to clean " + browserName + " cookies. The browser might be running - please close it and try again.");
    }
    cout << endl;
}

void main_menu_cookie() {
    Logger::info("Starting cookie cleanup menu");
    // Инициализация userProfile с обработкой ошибок
    if (FAILED(SHGetFolderPathA(NULL, CSIDL_PROFILE, NULL, 0, userProfile))) {
        Logger::error("Failed to get user profile path");
        printError("Failed to get user profile path");
        return;
    }
    
    Logger::success("User profile path obtained: " + string(userProfile));
