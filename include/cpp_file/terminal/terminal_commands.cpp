...
void checkForUpdatesOnce() {
    ...
    if (!updateCheckDone) {
        ...
        if (currentVersionUpper.find("DEV") != string::npos) {
            ...
            if (updateAvailable) {
                Logger::warning("Update available: " + latestVersion + " (current: " + VERSION + ")");
            } else {
                Logger::success("Program is up to date");
            }
        }
        updateCheckDone = true;
    } else {
        Logger::info("Update check already performed");
    }
}
...
string generateRandomHash(int length) {
    ...
    return hash;
    Logger::functions_log(LOG_INFO, "generateRandomHash", "Created hash: " + _hash_);
}
...
bool checkForUpdates(string& latestVersion, string& downloadUrl) {
    ...
    // Get latest release info from GitHub API using pipe
    string command = "curl -s https://api.github.com/repos/lackyhy/7SCW/releases/latest";
    FILE* pipe = popen(command.c_str(), "r");
    if (!pipe) {
        cout << "Error: Failed to execute curl command." << endl;
        return false;
    }

    // Read JSON response directly from pipe
    string jsonContent;
    char buffer[128];
    ...
    if (jsonContent.empty()) {
        cout << "Error: Failed to fetch update information. Check your internet connection." << endl;
        return false;
    }
